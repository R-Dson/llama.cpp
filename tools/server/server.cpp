...
struct server_task_result_cmpl_final : server_task_result {
    ...
    json to_json_oaicompat_api_chat() {
        ...
        json res = {
            ...
            { "done_reason",          finish_reason                                                           }, // Use ollama specific reason
            { "done",                 true                                                                    },
            // Add metrics from timings and other fields, converted to nanoseconds
            { "total_duration",       static_cast<uint64_t>((timings.prompt_ms + timings.predicted_ms) * 1e6) }, // ms to ns
            { "load_duration",
             static_cast<uint64_t>(timings.prompt_ms * 1e6)                                                   }, // Assuming load duration is prompt eval time
            { "prompt_eval_count",    n_prompt_tokens                                                         },
            { "prompt_eval_duration", static_cast<uint64_t>(timings.prompt_ms * 1e6)                          },
            { "eval_count",           n_decoded                                                               },
            { "eval_duration",        static_cast<uint64_t>(timings.predicted_ms * 1e6)                       }
        };
...
struct server_task_result_cmpl_partial : server_task_result {
    ...
    json to_json_oaicompat_api_chat() {
        // Ollama chat format is similar to OpenAI chat format, but with some differences
        std::time_t t = std::time(0);
        {
